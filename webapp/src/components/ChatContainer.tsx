...
const ChatContainer = ({ questions }: { questions: Array<Exchange> }) => {
  ...
  const aiPhoto = useMemo(() => {
    return screenState.darkMode ? aiDark : aiLight;
  }, [screenState.darkMode]);
  const studentPhoto = useMemo(() => {
    return screenState.darkMode ? studentLight : student;
  }, [screenState.darkMode]);
  ...
  useEffect(() => {
    if (
      commonlyAskedQuestionsRef !== null &&
      commonlyAskedQuestionsRef.current !== null
    ) {
  ...
  return (
    <Box
      sx={{ display: "flex", pb: 7, alignContent: "center", flexGrow: 1 }}
      ref={ref}
    >
      <CssBaseline />
      <Grid container>
        ...
        >
          {messages &&
            messages.map((msg) => (
              <ChatMessage
                key={"cm-" + Math.random().toString(2).slice(2) + msg.id}
                message={msg}
              />
            ))}
        ...
        <Grid item xs={12}>
          ...
          <Paper
            sx={{
              ...
              width: !shouldDisplayFAQ
                ? `calc(100% - ${isMobileView ? "50px" : "480px"})`
                : "40%",
            }}
            elevation={3}
          >
                ...
                endAdornment={
                  <>
                    {screenState.generativeMode ? (
                      <Tooltip
                        title={
                          'Although we make every effort to assure accuracy of responses to your questions. Still, we assume no responsibility or liability for any errors or omissions in the content of this site. The information contained in this site is provided on an "as is" basis with no guarantees of completeness, accuracy, usefulness or timeliness.'
                        }
                      >
        ...
        <Grid item xs={6} sx={{ display: shouldDisplayFAQ ? "" : "none" }}>
          ...
          >
            ...
            <Box>
              <List sx={{ m: 1 }}>{questionMenuItems}</List>
            </Box>
          </Drawer>
        </Grid>
      </Grid>
