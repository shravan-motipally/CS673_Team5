...
function Index() {
  ...
  const handleClassChange = useCallback(
    async (classSel: string) => {
      setClassSelected(classSel);
      const curClass = getClass(classSel);
      if (
        curClass !== undefined &&
        curClass?.courseId !== undefined &&
        curClass.courseId !== null
      ) {
  ...
  return (
    <React.Fragment>
      ...
      <Alert
        ...
        severity="warning"
      >
        Generation Capabilities added! Loading times are extended, apologies!
        Loading the backend now, please wait!
      </Alert>
      <LandingPage />
      <ThemeProvider theme={screenState.darkMode ? darkTheme : lightTheme}>
        ...
        >
          ...
          >
            ...
            {courseError ? (
              <FormHelperText>
                ⚠️ Error loading courses, refresh your page!
              </FormHelperText>
            ) : (
              <div />
            )}
